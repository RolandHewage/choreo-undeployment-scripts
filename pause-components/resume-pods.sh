#!/usr/bin/env bash
# Usage:
#   DEBUG=1 ./resume-pods.sh <org>-component-list.csv
#
# When DEBUG=1 it will print detailed info about each line and each curl.
#
# Scales the paused component workloads back up via dp-rudder's /resume
# endpoint. Mirror of pause-pods.sh — use as the rollback for a prior pause.
...
OUTPUT_FILE="resume-results-$(date -u +%Y%m%d-%H%M%SZ).csv"
...
# CSV format: component,component_id,release_id
tail -n +2 "$INPUT_FILE" | while IFS=, read -r component component_id release_id; do
  ...
  url="$BASE_URL/components/$component_id/releases/$release_id/resume"
